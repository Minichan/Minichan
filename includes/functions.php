...
function sendMessageToPhone($deviceRegistrationId, $msgType, $extraParams) {
	...
	if(!defined('GOOGLE_TOKEN')) return;
...
//	return false;
	...
	if(!is_array($gold_accounts)) $gold_accounts = array();
	if(in_array($uid, $gold_accounts)) return true;
...
function detect_spam($haystack){
	...
	foreach($arrChars as $to=>$chars) $haystack = str_replace($chars, $to, $haystack);
	foreach($spam_phrases as $phrase){
		if(detect_phrase($phrase, $haystack)) return true;
	}
	return false;
}
...
function recaptcha($extra=''){
	...
	foreach($_POST as $k=>$v){ // Let the user resume as intended
		if($k=='recaptcha_challenge_field'||$k=='recaptcha_response_field') continue;
...
, $uid);
	...
	while(list($trash_headline, $trash_body, $id, $parent_id, $trash_time, $mod_uid) = $link->fetch_row($fetch_trash)) {
		...
		if(!allowed("open_profile")) unset($values[3]);
...
function isMatt($ip) {
	...
	foreach($chinaCIDRS as $cidr) {
		if(ipCIDRCheck($_SERVER['REMOTE_ADDR'], $cidr)) return true;
	}
	if(strpos($_SERVER['REMOTE_ADDR'], '65.49.') === 0) return true;
	if($hostname && $hostname != '.' && substr_compare($hostname, $test, -strlen($test), strlen($test)) === 0) return true;

	return false;
}

function isHindu($ip) {
	if($ip == "100.33.0.74") return true;
	return false;
}

function getFlagForIP($ip) {
	if(isMatt($ip)) return 'china';
	return null;
}
